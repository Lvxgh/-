...
命令一览：
    ...
    python rag_cli.py ask    <问题>           # 检索 + 生成回答
                                              #   --backend deepseek（默认，需 DEEPSEEK_API_KEY）
                                              #   --backend claude（需 ANTHROPIC_API_KEY）
...
# 若 API 报"model not found"，改用官方滚动别名 --model deepseek-chat（始终指向当前主力模型）
...
def main():
    ...
    p_ask.add_argument(
        "--backend", choices=["claude", "deepseek", "ollama"], default="deepseek",
        help="deepseek=DeepSeek API（默认，便宜）；claude=Claude API；ollama=本地模型，完全离线",
    )
    ...
    p_mext.add_argument("--backend", choices=["claude", "deepseek", "ollama"], default="deepseek",
                        help="deepseek=DeepSeek API（默认，便宜）；claude=Claude API；ollama=本地模型")
